...
void main(void)
{
   ...
   TCCR1B = (0 << ICNC1) | (0 << ICES1) | (0 << WGM13) | (0 << WGM12) | (1 << CS12) | (0 << CS11) | (1 << CS10); //�Ϲݸ��, ���������� = CK/1024
   TCNT1H = 0x00;
   TCNT1L = 0x00;
   ICR1H = 0x00;
   ICR1L = 0x00;
...
interrupt[USART0_RXC] void UART_RX(void)
{
   ...
   if (rx == 'R') // 'R'을 받으면 com_error=0으로 변경.
   {
      com_error = 0;
      return;
   }
...
void stringtohex(void)
{
   ...
   {
      Putch(as_rx[0]); // data return
      for (i = 0; i < 3; i++)
      {
         Putch(as_rx[i + 1]);    // data return
...
/*
Q // 데이터 없고 대기상태 유지 
Ve0000000 // V Valve on-off + onoff 신호 (밸브 모두를 2진수로 30개 밸브 8자리 1,2,3번 켜기)          
M111 (Pump motor 3�� ���� �ѱ� )
M111
M111
M111
...
V00000000  //  Valve off
V1c000000  0001 1100... 4,5,6 벨브 키라는 명령
M111
M111
M111
M111
